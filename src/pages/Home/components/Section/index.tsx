...
interface CategoryItem {
    ...
    coursesList: CourseOfCategory[]
}

function Section({ title, description, backgroundColor, urlCourses, ultils, footer, sectionId }: Props) {
    const stateSection = useSelector((state: RootState) => state.category[sectionId])
    const listIds = stateSection ? stateSection.listIds : []
    const coursesOfCategoryId = stateSection ? stateSection.coursesList : []
                                ...
                                <div className={cx('content-category')}>
                                    <Grid container spacing={2}>
                                        {coursesOfCategoryId[tabActive] &&
                                            coursesOfCategoryId[tabActive][0].coursesList.map(
                                                (course: CourseOfCategory, index: number) => (
                                                    <Grid
                                                        key={index}
                                                        item
                                                        xs={6}
